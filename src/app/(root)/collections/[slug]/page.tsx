...
import { getCollectionBySlug } from "@/lib/actions/collection";
...
export default async function CollectionDetailPage({
  ...
  parsed.collectionSlugs = [slug]; // Force collection filter
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      <div className="py-4 text-sm text-dark-700">
        <Link href="/collections" className="hover:text-dark-900 transition-colors">
          &larr; Back to Collections
        </Link>
      </div>
      
      <header className="flex items-center justify-between py-6">
        <h1 className="text-heading-3 text-dark-900">{collection.name} ({totalCount})</h1>
        <Sort />
      </header>
